	...
	<head>
		...
		<script type="text/javascript">
			$(document).ready(function () {
				...
				var source = [
					{ icon: "imagens/dirid.png", label: "DIRID", value:"reqFaturamentoDirid", expanded: true, items: 
						[
			...
			//		var url = "visaoExecutivaDiaria?ano="+ano+"&tipo=";
			...
			function trocaGrafico(ano,produto){
				...
				if(ano !== "zero" && produto !== "zero"){
					...
					$.ajax({
						// have to use synchronous true, else the function 
						...
						error: function(XMLHttpRequest, textStatus, errorThrown) {
					        ...
					        $('#tabelaAjax').html("<span style='color:red;margin-left: 136px;'>Em manutenção!.</span><br>");
					    }    
					});
				}
			}
			...
			function baixarBaseSinistrosPendentes(){
				var produto = $('#jqxTree').jqxTree('getSelectedItem');
				var prod = produto.value;
				var id = document.getElementById("selectForm");
				var url = "visaoExecutivaDiaria?tipo=";
				url = "http://"+ip_porta+"/ProjetoCaixa/"+url+prod+"&categ=baseSinistroPendenteFaixa";
				//alert(url);
	  			window.open(url,"_blank","scrollbars=yes,resizable=yes,top=100,left=70,width=500,height=500");
			}
			
			function baixarBaseSinistrosCompleta(){
				var produto = $('#jqxTree').jqxTree('getSelectedItem');
				var prod = produto.value;
				var ano = $('#selectAno').val();
				var url = "visaoExecutivaDiaria?tipo=";
				url = "http://"+ip_porta+"/ProjetoCaixa/"+url+prod+"&ano="+ano+"&categ=baseSinistroCompleta";
				//alert(url);
	  			window.open(url,"_blank","scrollbars=yes,resizable=yes,top=100,left=70,width=500,height=500");
			}
			
			function baixarBaseFaturamentoRo(){
				var produto = $('#jqxTree').jqxTree('getSelectedItem');
				var prod = produto.value;
				var ano = $('#selectAno').val();
				var url = "visaoExecutivaDiaria?tipo=";
				url = "http://"+ip_porta+"/ProjetoCaixa/"+url+prod+"&ano="+ano+"&categ=baseFaturamentoRO";
				//alert(url);
				//return url;
	  			window.open(url,"_blank","scrollbars=yes,resizable=yes,top=100,left=70,width=500,height=500");
			}
	...
	<body role="document" background="imagens/fundo.jpg">
		<nav class="navbar navbar-inverse navbar-fixed-top">
			<div class="container">
				...
				<div id="navbar" class="navbar-collapse collapse">
					...
					<!-- ul class="nav navbar-nav">
						<li id="botaoDiario">
							<a href="pagiGraficoMes.jsp">Acompanhamento Mensal</a>
						</li>
					</ul -->
				</div>
			</div>
		...
		<div class="container theme-showcase" role="main" style="padding-left: 0px;">
			...
			<div class="page-header">
				<h2>Gest&atilde;o &agrave; Vista - Faturamento - <span style="color:red;">Acompanhamento Di&aacute;rio</span></h2>
				<!--  <br>
				<h3><small>Autom&oacute;vel - Faturamento</small></h3>-->
			</div>
			<form action="visaoExecutivaDiaria?tipo=" id="selectForm">
