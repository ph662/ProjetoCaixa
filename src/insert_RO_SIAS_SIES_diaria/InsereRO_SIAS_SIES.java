...
public class InsereRO_SIAS_SIES {
	...
	public static void main(String[] args) {
		...
		// for (int i = 3; i <= 25; i++) {
		//
		// if (i == 8 || i == 9 || i == 15 || i == 16 || i == 22 || i == 23) {
		// System.out.println("pulou o dia " + i);
		// continue;
		// }
		//
		// if (i < 10) {
		// anoCompletoMes = anoMesDia_M2087 = "2017070" + i;
		// } else {
		// anoCompletoMes = anoMesDia_M2087 = "201707" + i;
		// }
		...
		try {
			...
			anoCompletoMes = anoMesDia_M2087 = "20170727";
			...
			/*
			 * // apenas no primeiro dia util o RG0012B.TXT eh gerado // se for
			 * primeiro dia util if
			 * (DATE_FORMAT4.format(anoCompletoMes_Calendar.getTime())
			 * .equalsIgnoreCase( uteis.getPrimeiroDiaUtilDoMes(
			 * anoCompletoMes_Calendar, 4))) { // SE A DATA 'anoCompletoMes' QUE
			 * FOR RODAR A ROTINA FOR O // PRIMEIRO DIA UTIL DO MES, APAGA O
			 * GL52B E INSERE O RO DE // FECHAMENTO DE SINISTROS RORG52(mes
			 * anterior) // insereRG0052B_Mensal(anoCompletoMes);
			...
			e.printStackTrace();
			System.out
					.println("Insira o parametro 'anoMesDia' no seguinte formato 'AAAAmmDD'");
		}
	...
	private static void insereRoSIASdiario(String anoCompletoMes)
			throws SQLException {

		String YYYYmm = anoCompletoMes.substring(0, 6);
		String YYmmdd = anoCompletoMes.substring(2, 8);

		File file = new File("\\\\ntservercpd\\Registros_OFICIAIS\\D" + YYmmdd
				+ ".GL0012B.TXT");
		...
		try {
			...
			if (file.exists()) {
				...
				Connection con = new Conexao()
						.getConexaoMySql("insereRoSIAS_SIES.java - \\\\ntservercpd\\Registros_OFICIAIS\\D"
								+ YYmmdd + ".GL0012B.TXT");
				...
				System.out.println("Arquivo GL0012B SIAS não encotrado.");
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		try {
			Connection con = new Conexao()
					.getConexaoMySql("insereRoSIAS_SIES.java - prcSiasFiltraGL12Faturamento()");
			Statement statement = con.createStatement();
			/* Faz um select simples na tabela temp_graficoFaturamento */

			String insertSelect = "call `prcSiasFiltraGL12Faturamento`('"
					+ YYYYmm + "')";
	...
	private static void insereRoSinistroSIASdiario(String anoCompletoMes)
			throws SQLException {
		...
		try {
			if (file.exists()) {
				...
				try {
					...
					System.out
							.println("Problema em insereROSinistroSIASdiario");
					e.printStackTrace();
				}
				...
				while (dis.available() != 0) {
					...
					if (part[0].matches("\\d+")) {
						...
						if (part[1].equalsIgnoreCase("0114")
								|| part[1].equalsIgnoreCase("0118")
								|| part[1].equalsIgnoreCase("0351")
								|| part[1].equalsIgnoreCase("0141")
								|| part[1].equalsIgnoreCase("0167")
								|| part[1].equalsIgnoreCase("0171")
								|| part[1].equalsIgnoreCase("0520")
								|| part[1].equalsIgnoreCase("0553")
								|| part[1].equalsIgnoreCase("0531")
								|| part[1].equalsIgnoreCase("0542")) {
							...
							try {
								/*
								 * insert que será feito na tabela de
								 * 'ro_sinistro_sias_diaria'
								 */
								...
								for (i = 1; i <= 23; i++) {
									...
									if (i <= 4) {
										...
										ps.setDouble(i + 1, Double
												.parseDouble(part[i - 1]
														.replace(".", "")
														.replace(",", ".")));
									} else {
				...
				System.out.println("Arquivo GL0052B SIAS não encotrado.");
			}// if file exist
	 ...
	 * movimentos nao estao nele. Para isso este metodo apaga o movimento do GL
	 * que esta sendo inserido diariamente e insere o conteudo do arquivo de
	 * fechamento de sinistros mensal 'RG0052B_DYYmmdd.TXT'
	 */
	private static void insereRG0052B_Mensal(String anoCompletoMes)
			throws SQLException {
		...
		File file = new File("\\\\ntservercpd\\Registros_OFICIAIS\\RG0052B_D"
				+ YYmmdd + ".TXT");
	...
	private static void insereRoEmissaoSIESdiario(String anoCompletoMes,
			String anoMesDia_M2087) throws SQLException {
		...
		File file = new File(
				"\\\\ntservercpd\\SIES\\DADOS_GECONT\\RO\\ROEMTLDR.D" + YYmmdd
						+ ".csv");
		...
		try {
			...
			if (file.exists()) {
				...
				Connection con = new Conexao()
						.getConexaoMySql("InsereRO_SIES.java - \\\\ntservercpd\\SIES\\DADOS_GECONT\\RO\\ROEMTLDR.D"
								+ YYmmdd + ".csv");
				Connection con2 = new Conexao()
						.getConexaoMySql("InsereRO_SIES.java insereRoEmissaoSIESdiario");

				System.out.println("Inserindo linhas...");

				String deleteROSIES = "delete from ro_sies_diaria where anomes_ref = '"
						+ YYYYmm + "' AND movimento = 'APOLICE DE SEGURO';";
				PreparedStatement psDel = con.prepareStatement(deleteROSIES);
				psDel.executeUpdate();

				String deleteROSIES_uf = "delete from ro_uf_sies_diaria where anomes_ref = '"
						+ YYYYmm + "'";
				PreparedStatement psDel_uf = con
						.prepareStatement(deleteROSIES_uf);
						...
						try {
							...
							for (i = 1; i <= 34; i++) {
								...
								case 3: // RAMO
									...
									psUF.setDouble(i,
											Double.parseDouble(part[8].trim()));// apolice
									break;
								case 4: // PRODUTO
									ps.setString(i, part[6]);// produto
									psUF.setDouble(i,
											Double.parseDouble(part[9].trim()));
									break;
								case 5: // MOVIMENTO
								...
								case 6: // APOLICE
									...
									if (M2087existe) {
										...
										for (int j = 0; j < listaM20.size(); j++) {
											...
											if (part[8].equals(listaM20.get(j)
													.getApolice())) {
												canalVenda = listaM20.get(j)
														.getCanal();
												agenciaVenda = Integer
														.parseInt(listaM20.get(
																j).getAgencia());
												break;
											} else {
												canalVenda = "-";
										...
										canalVenda = "m2087_" + YYmmdd_M2087
												+ "_NaoEncontrado";
										agenciaVenda = 0;
									}
	...
	private static void insereRoCancelamentoSIESdiario(String anoCompletoMes)
			throws SQLException {
		...
		File file = new File(
				"\\\\ntservercpd\\SIES\\DADOS_GECONT\\RO\\ROCANLDR.D" + YYmmdd
						+ ".csv");
		...
		try {
			...
			if (file.exists()) {
				...
				System.out
						.println("Arquivo RO SIES Cancelamento não encontrado.");
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private static void insereRoRestituicaoSIESdiario(String anoCompletoMes)
			throws SQLException {
		...
		File file = new File(
				"\\\\ntservercpd\\SIES\\DADOS_GECONT\\RO\\RORSTLDR.D" + YYmmdd
						+ ".csv");
		...
		try {
			...
			if (file.exists()) {
				...
				System.out
						.println("Arquivo RO SIES Restituicao não encontrado.");
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private static void insereRoSinistroSIESdiario(String anoCompletoMes)
			throws SQLException {
		String YYYYmm = anoCompletoMes.substring(0, 6);
		String YYmmdd = anoCompletoMes.substring(2, 8);

		File file = new File(
				"\\\\ntservercpd\\SIES\\DADOS_GECONT\\RO\\ROSINLDR.D" + YYmmdd
						+ ".csv");
		...
		try {
			...
			if (file.exists()) {
				...
				String deleteROSIES = "delete from ro_sinistro_sies_diaria where anomes_ref = '"
						+ YYYYmm + "';";
				PreparedStatement psDel = con.prepareStatement(deleteROSIES);
				psDel.executeUpdate();
						...
						try {
							...
							for (i = 1; i <= 17; i++) {
								if (i == 1) {// YYYYmm
									...
									ps.setString(i, part[i - 2].replaceAll(
											"\\s\\d{2}:\\d{2}:\\d{2}", ""));
	...
	private static void excluiDadosTemp_graficofaturamento(String anoCompletoMes) {
		...
		Connection con = new Conexao()
				.getConexaoMySql("insereRoSIAS_SIES.java - excluiDadosTemp");
		System.out.println("\nDeletando dados do ano de " + YYYY
				+ " da tabela temp_graficofaturamento_diaria...");
		try {
			String sqlDelete = "DELETE from temp_graficofaturamento_diaria where ano = '"
					+ YYYY + "'";
			PreparedStatement ps = con.prepareStatement(sqlDelete);
			ps.execute();
			ps.close();
			ps = null;
			System.out
					.println("Dados da temp_graficofaturamento_diaria deletados.\n");
		} catch (Exception e) {
			System.out
					.println("Problema em - excluiDadosTemp - InsereRO_SIAS_SIES");
			e.printStackTrace();
		} finally {
			try {
				con.close();
				...
				e.printStackTrace();
				System.out
						.println("ERRO metodo excluiDadosTemp InsereRO_SIAS_SIES");
			}
		}
	}

	private static void excluiDados_EmissaoPorCanalVenda(String anoCompletoMes) {
		String YYYY = anoCompletoMes.substring(0, 4);
		Connection con = new Conexao()
				.getConexaoMySql("insereRoSIAS_SIES.java - excluiDados_EmissaoPorCanalVenda");
		System.out.println("\nDeletando dados do ano de " + YYYY
				+ " da tabela temp_graficofaturamento_emissaoporcanal...");
		try {
			String sqlDelete = "DELETE from temp_graficofaturamento_emissaoporcanal where ano = '"
					+ YYYY + "'";
			PreparedStatement ps = con.prepareStatement(sqlDelete);
			ps.execute();
			ps.close();
			ps = null;
			System.out
					.println("Dados da temp_graficofaturamento_emissaoporcanal deletados.\n");
		} catch (Exception e) {
			System.out
					.println("Problema em - excluiDados_EmissaoPorCanalVenda - InsereRO_SIAS_SIES");
			e.printStackTrace();
		} finally {
			try {
				con.close();
				...
				e.printStackTrace();
				System.out
						.println("ERRO metodo excluiDados_EmissaoPorCanalVenda InsereRO_SIAS_SIES");
			}
		}
	}

	private static void atualizaDadosTemp_graficoFaturamento(
			String anoCompletoMes) {
		String YYYY = anoCompletoMes.substring(0, 4);

		System.out.println(new Date(System.currentTimeMillis())
				+ " Método atualizaDadosTemp: Inserindo dados na tabela...");

		Connection con = new Conexao()
				.getConexaoMySql("insereRoSIAS_SIES.java - atualizaDadosTemp");
		...
		try {
			...
			String insereAutoTranqExcl = "call `prcDados_DIARIA_Faturamento`('"
					+ YYYY
			...
			String insereAutoTranqCorren = "call `prcDados_DIARIA_Faturamento`('"
					+ YYYY
			...
			/* AUTO Frota */
			String insereAutoFrota = "call `prcDados_DIARIA_Faturamento`('"
					+ YYYY + "','Auto Tranquilo Frota','3120,3172')";
			statement.executeUpdate(insereAutoFrota);

			/* AUTO Facil */
			String insereAutoFacil = "call `prcDados_DIARIA_Faturamento`('"
					+ YYYY + "','Auto Fácil','5302,5303,5304,5388')";
			...
			/* RDEquipamentos */
			String insereRdEquipamentos = "call `prcDados_DIARIA_Faturamento`('"
					+ YYYY + "','Rd Equipamentos','7114')";
			statement.executeUpdate(insereRdEquipamentos);

			/* EMPRESARIAL */
			String insereEmpresarial = "call `prcDados_DIARIA_Faturamento`('"
					+ YYYY + "','MR Empresarial','1801,1802,1804')";
			statement.executeUpdate(insereEmpresarial);

			/* LOTERICO */
			String insereLoterico = "call `prcDados_DIARIA_Faturamento`('"
					+ YYYY + "','Loterico','1803')";
			statement.executeUpdate(insereLoterico);

			/* CCA */
			String insereCca = "call `prcDados_DIARIA_Faturamento_Loterico_CCA`('"
					+ YYYY + "','CCA','%CCA%','1805')";
			...
			/* Facil Residencial */
			String insereFacilRD = "call `prcDados_DIARIA_Faturamento`('"
					+ YYYY + "','Facil Residencial','1402,1405')";
			statement.executeUpdate(insereFacilRD);

			/* MR Residencial Correntista */
			String insereRdCorrentista = "call `prcDados_DIARIA_FaturamentoRdCorrentista`('"
					+ YYYY + "','MR Residencial Correntista')";
			statement.executeUpdate(insereRdCorrentista);

			/* MR Residencial Economiario */// exclusivo
			String insereRdExclusivo = "call `prcDados_DIARIA_Faturamento`('"
					+ YYYY + "','MR Residencial Economiario','1404')";
			statement.executeUpdate(insereRdExclusivo);

			/* Lar Mais */
			String insereLarMais = "call `prcDados_DIARIA_Faturamento`('"
					+ YYYY + "','Lar Mais','1409')";
			...
			/* RD PF Outros */
			String insereRdPfOutros = "call `prcDados_DIARIA_Faturamento`('"
					+ YYYY
			 ...
			 */
			/* AUTO */
			String insereAuto = "call `prcDados_DIARIA_FaturamentoAutomovel`('"
					+ YYYY + "','AUTOMOVEIS')";
			statement.executeUpdate(insereAuto);

			/* RD PJ */
			String insereRDPJ = "call `prcDados_DIARIA_Faturamento_RDPJ`('"
					+ YYYY + "','RDPJ')";
			statement.executeUpdate(insereRDPJ);

			/* RD PF */
			String insereRDPF = "call `prcDados_DIARIA_Faturamento_RDPF`('"
					+ YYYY + "','RDPF')";
			statement.executeUpdate(insereRDPF);

			/* DIRID */
			String insereDIRID = "call `prcDados_DIARIA_FaturamentoDirid`('"
					+ YYYY + "','Dirid')";
			statement.executeUpdate(insereDIRID);

			System.out.println(new Date(System.currentTimeMillis())
					+ " Método atualizaDadosTemp: ...dados inseridos.");

		} catch (Exception e) {
			System.out
					.println("ERRO metodo atualizaDadosTemp InsereRO_SIAS_SIES");
			e.printStackTrace();
		}
	}
	...
	private static void atualizaHistoricoTemp(String anoCompletoMes) {
		...
		Connection con = new Conexao()
				.getConexaoMySql("InsereRO_SIAS_SIES.java");
		try {
			...
			try {
				...
				e.printStackTrace();
				System.out
						.println("ERRO metodo atualizaHistoricoTemp InsereRO_SIAS_SIES");
			}
		}
	}

	private static void insereRoSIESdiario(String anoCompletoMes,
			String anoMesDia_M2087) throws SQLException {
		insereRoEmissaoSIESdiario(anoCompletoMes, anoMesDia_M2087);
		insereEmissaoPorCanalVenda(anoCompletoMes);
		insereRoCancelamentoSIESdiario(anoCompletoMes);
		insereRoRestituicaoSIESdiario(anoCompletoMes);
	}

	private static void insereEmissaoPorCanalVenda(String anoCompletoMes) {
		excluiDados_EmissaoPorCanalVenda(anoCompletoMes);
		String YYYY = anoCompletoMes.substring(0, 4);
		System.out
				.println(new Date(System.currentTimeMillis())
						+ " Método insereEmissaoPorCanalVenda: Inserindo dados na tabela...");

		Connection con = new Conexao()
				.getConexaoMySql("insereRoSIAS_SIES.java - insereEmissaoPorCanalVenda");
		System.out.println("Executando as procedures...");
		try {
			Statement statement = con.createStatement();
			/* ==================================== */
			...
			/* RDEquipamentos */
			String insereRdEquipamentos = "call `prcDados_DIARIA_EmissaoPorCanal`('"
					+ YYYY + "','Rd Equipamentos','7114')";
			statement.executeUpdate(insereRdEquipamentos);

			/* EMPRESARIAL */
			String insereEmpresarial = "call `prcDados_DIARIA_EmissaoPorCanal`('"
					+ YYYY + "','MR Empresarial','1801,1802,1804')";
			statement.executeUpdate(insereEmpresarial);
			...
			/* Facil Residencial */
			String insereFacilRD = "call `prcDados_DIARIA_EmissaoPorCanal`('"
					+ YYYY + "','Facil Residencial','1402,1405')";
			statement.executeUpdate(insereFacilRD);

			/* MR Residencial Correntista */
			String insereRdCorrentista = "call `prcDados_DIARIA_EmissaoPorCanal`('"
					+ YYYY + "','MR Residencial Correntista','1403,1407')";
			statement.executeUpdate(insereRdCorrentista);

			/* MR Residencial Economiario */// exclusivo
			String insereRdExclusivo = "call `prcDados_DIARIA_EmissaoPorCanal`('"
					+ YYYY + "','MR Residencial Economiario','1404')";
			statement.executeUpdate(insereRdExclusivo);

			/* RD PF Outros */
			String insereRdPfOutros = "call `prcDados_DIARIA_EmissaoPorCanal`('"
					+ YYYY
					+ "','Rd Pf Outros','1002,1410,1411,1489,4002,7100,7101,7106,7107,7108,7109,7110,7117,7122,7123')";
			statement.executeUpdate(insereRdPfOutros);
			...
			System.out
					.println("ERRO metodo insereEmissaoPorCanalVenda InsereRO_SIAS_SIES");
			e.printStackTrace();
		}
	}
	...
	public static void insereM2087(String anoCompletoMes) throws SQLException {
		...
		try {
			...
			if (file.exists()) {
				...
				String deleteM2087 = "delete from sies_m2087 where anomes_ref = "
						+ yyyyMM + ";";
				...
				System.out.println("Arquivo SIES_M2087_" + yyMMdd
						+ ".txt não encontrado.");
			}
		} catch (IOException e) {
			e.printStackTrace();
