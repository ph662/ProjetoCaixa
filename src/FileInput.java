...
public class FileInput {
	...
	public static void main(String[] args) {
		...
		/********************************************************************
		 * Neste trecho é feita a declaração do local do arquivo, os arquivos
		 * comentados são arquivos que já foram usados.
		 */
		// File file = new File("D:\\D151007_RelAcompRenovacao.csv");//setembro
		File file = new File(
				"\\\\ntservercpd\\GERID\\D170731_RelAcompRenovacao.csv");
		...
		try {
			...
			while (dis.available() != 0) { // Enquanto não for o fim do arquivo
				...
				/*
				 * A Linha atual é cortada utilizando o delimitador e atribuida
				 * a um array String
				 */
				String[] part = dis.readLine().split(";");
				/********************************************************************
				 * Neste trecho é feita a declaração de como será o INSERT, os
				 * arquivos comentados são arquivos que já foram usados. A
				 * quantidade de colunas declaradas aqui deve ser a mesma do
				 * 'for' mais abaixo.
				 */

				// /*Arquivo "M1504.RG0012B.txt" 41 Colunas*/
				// String sql =
				// "INSERT INTO SistemaDirid.MesAbril_RG0012B (ORG,RAMO,PROD,MOVIMENTO,DIA,FONTE,APOLICE,ENDOSSO,END_MOVTO,PARC,INIC_VIG,TERM_VIG,SEGURADO,CNPJ_CPF,TIPO_INDEX,DT_VENCTO,IMP_SEGURADA_LIDER,IMP_SEG_COSSEG_CEDIDO,IMP_SEG_RESSEG_CEDIDO,PREM_TARIFARIO_LIDER,PRM_TARIF_COSSEG_CED,PRM_TARIF_RESSEG_CED,VLR_DESCTO_LIDER,DESCTO_COSSG_CED,DESCTO_RESSG_CED,PREMIO_LIQUIDO_LIDER,PREM_LIQD_COSSEG_CED,PREM_LIQD_RESSEG_CED,VLR_ADIC_LIDER,ADIC_COSSG_CED,ADIC_RESSG_CED,VLR_CUSTO_EMISS,VALOR_DO_IOF,VALOR_PREMIO_TOTAL,VLR_COMIS_CORRT,VLR_COMIS_ADMN,VLR_COMIS_AGENC,VLR_COMIS_COSSG,VLR_COMIS_RESSG,PONTO_DE_VENDA) VALUES (?,?,?,?,?,?,?,?,?,?,STR_TO_DATE(?,'%d/%m/%Y'),STR_TO_DATE(?,'%d/%m/%Y'),?,?,?,STR_TO_DATE(?,'%d/%m/%Y'),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
				// /**************************/
				...
				// String sql =
				// "INSERT INTO SistemaDirid.MesAbril_ROEMTLDR(LIVRO,PAGINA,ORGAO,COD_RAMO,RAMO,MES_DE_COMPETENCIA,PRODUTO,UF,APOLICE,ENDOSSO,CERTIFICADO,PARCELA,MOEDA,DT_EMISSAO,DT_INICIO,DT_TERMINO,CPF_CNPJ,SEGURADO,COD_SUSEP,CORRETOR,LIDER_IMP_SEGURADA,COSS_IMP_SEGURADA,RESS_IMP_SEGURADA,LIDER_PREMIO_TARIF,COSS_PREMIO_TARIF,RESS_PREMIO_TARIF,LIDER_DESCONTO,COSS_DESCONTO,LIDER_ADICIONAL,COSS_ADICIONAL,LIDER_CUSTO,LIDER_IOF,LIDER_COMISSAO,COSS_COMISSAO,RESS_COMISSAO,LIDER_PREMIO_TOTAL,COSS_PREMIO_TOTAL,RESS_PREMIO_TOTAL,REEMISSÃO) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,STR_TO_DATE(?,'%d/%m/%Y'),STR_TO_DATE(?,'%d/%m/%Y'),STR_TO_DATE(?,'%d/%m/%Y'),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
				...
				// String sql =
				// "INSERT INTO sistemadirid.EF_HABIT0000_PREMIOS VALUES (?,?,?,?,?,?,?,STR_TO_DATE(?,'%d/%m/%Y'),STR_TO_DATE(?,'%d/%m/%Y'),STR_TO_DATE(?,'%d/%m/%Y'),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";

				/* Arquivo "extracao2011_RO_RG.rpt" 39 Colunas */
				// String sql =
				// "INSERT INTO sistemadirid.RO_RG0012B VALUES (?,?,?,?,?,?,?,?,?,?,?, STR_TO_DATE(?,'%Y-%m-%d'), STR_TO_DATE(?,'%Y-%m-%d'),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
				...
				// String sql =
				// "insert into sistemadirid.agencia_vigentes (NumeroDaProposta,NumeroDaAgenciaDeVenda,CodigoProduto) values(?,?,?);";
				...
				// String sql =
				// "INSERT INTO agencia_vigentes_apolices(DataArquivo,CodigoProduto,`NumeroApolice`,`AgenciaDeVenda`,`NumeroDaSuperintendenciaRegional`,`NumeroDaFilial`) VALUES (STR_TO_DATE(?,'%d-%m-%Y'),?,?,?,?,?)";

				/* Arquivo "vendaSumarizado.csv" 3 Colunas */
				// String sql =
				// "insert into agencia_vigentes(DataArquivo,NumeroProposta, CodigoProduto, AgenciaDeVenda) values (?,?,?,?)";

				// String sql =
				// "insert into movimentacao_financeira(DataArquivo,NumeroProposta,CodigoSituacao,ValorParcela) values (?,?,?,?);";
				// String sql =
				// "insert into movimentacao_emissao (DataArquivo,NumeroProposta,AgenciaDeVenda,ValorPagoSICOB,SituacaoProposta,TipoMovimento,NumeroProdutoSIGPF) values (STR_TO_DATE(?,'%d-%m-%Y'),?,?,?,?,?,?);";
				// String sqlAgenciaVigentes =
				// "insert into agencia_vigentes (DataArquivo,NumeroProposta,AgenciaDeVenda,CodigoProduto) values (STR_TO_DATE(?,'%d-%m-%Y'),?,?,?);";
				...
				// String sql =
				// "insert into movimentacao_financeira (DataArquivo,NumeroProposta,CodigoSituacao,ValorParcela) values (STR_TO_DATE(?,'%d-%m-%Y'),?,?,?)";
				...
				try {
					...
					/*
					 * Troque o número que limita este 'for' de acordo com as
					 * colunas do INSERT
					 */

					/*
					 * for (int i = 1; i <= 16; i++) { if (i == 1) {
					 * ps.setInt(i, Integer.parseInt(part[i - 1].trim()
					 * .replaceAll("COD_AG", "9999"))); } else { ps.setString(i,
					 * part[i - 1].trim()); } }
					 */
